...
const polarOrderEvents = new Set(["order.paid", "order.refunded"]);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      return json({ ok: true, service: "mac-kit-polar-webhook", providers: ["polar", "paddle"] });
    }
...
// Polar signs with Standard Webhooks: HMAC-SHA256 over "<id>.<timestamp>.<body>"
// keyed by the base64-decoded secret (after "whsec_"). Secrets issued before
// 8 Sept 2026 used the raw secret string as the key instead, so both are tried.
async function handlePolarWebhook(request, env, ctx) {
  ...
  const signatureResult = await verifyPolarSignature({
    rawBody,
    webhookId,
    webhookTimestamp,
    signatureHeader,
    secret: env.POLAR_WEBHOOK_SECRET,
    toleranceSeconds: Number(env.POLAR_SIGNATURE_TOLERANCE_SECONDS || 300),
  });
  ...
  const handledKey = `polar:event:${webhookId}:handled`;
  const receivedKey = `polar:event:${webhookId}:received`;
  const alreadyHandled = await env.PADDLE_EVENTS.get(handledKey);

  if (alreadyHandled) {
    return json({ ok: true, duplicate: true, event_id: webhookId });
  }
  ...
  // Polar retries on anything but a fast 2xx and disables the endpoint after
  // ten failures in a row, so the work happens after the response.
  ctx.waitUntil(processPolarEvent(event, webhookId, env, handledKey));

  return json({ ok: true, event_id: webhookId, event_type: event.type });
}

async function processPolarEvent(event, webhookId, env, handledKey) {
  let processError = null;

  try {
    if (event.type === "order.paid") {
      await storePolarOrder(event, webhookId, env);
    } else if (event.type === "order.refunded") {
      await refundPolarOrder(event, webhookId, env);
    } else if (polarSubscriptionEvents.has(event.type)) {
      await storePolarSubscription(event, webhookId, env);
    }
  } catch (error) {
...
// The order carries the customer inline, so no API round trip is needed.
// Renewals (billing_reason subscription_cycle) land on the existing license
// through the subscription index inside ensureLicenseForPurchase.
async function storePolarOrder(event, webhookId, env) {
  ...
  const purchase = {
    event_id: webhookId,
    event_type: event.type,
    occurred_at: event.timestamp || order.created_at || null,
    transaction_id: orderId,
    customer_id: order.customer_id || order.customer?.id || null,
    customer_email: normalizeEmail(order.customer?.email),
    subscription_id: order.subscription_id || null,
    status: order.status || null,
    billing_reason: order.billing_reason || null,
    currency_code: order.currency || null,
    total: Number.isFinite(order.total_amount) ? order.total_amount / 100 : null,
    plan: polarPlan(order, env),
    price_ids: order.product_id ? [order.product_id] : [],
    custom_data: order.metadata || {},
    source: "polar",
    stored_at: now,
  };

  await env.PADDLE_EVENTS.put(`purchase:${orderId}`, JSON.stringify(purchase));

  if (order.subscription_id) {
    await env.PADDLE_EVENTS.put(
      `subscription:${order.subscription_id}:latest_purchase`,
      JSON.stringify(purchase)
    );
  }

  await ensureLicenseForPurchase(purchase, order, env);
}

// Only a full refund of a one-time order closes the license. Subscription
// orders are left alone: Polar ends access through subscription.revoked.
async function refundPolarOrder(event, webhookId, env) {
  const order = event.data || {};
  if (!order.id) return;
  ...
  const fullRefund = Number.isFinite(total) && total > 0 && refunded >= total;

  await env.PADDLE_EVENTS.put(
    `purchase:${order.id}:refund`,
    JSON.stringify({
      event_id: webhookId,
      occurred_at: event.timestamp || null,
      refunded_amount: Number.isFinite(refunded) ? refunded / 100 : null,
      total: Number.isFinite(total) ? total / 100 : null,
      full_refund: fullRefund,
      stored_at: now,
    })
  );

  if (!fullRefund || order.subscription_id) return;

  const license = await getLicenseByIndex(`license_by_transaction:${order.id}`, env);
  if (!license) return;

  license.status = "refunded";
  license.refunded_at = event.timestamp || now;
  license.updated_at = now;
  await storeLicense(license, env);
}

async function storePolarSubscription(event, webhookId, env) {
  ...
  const subscriptionSummary = {
    ...
    customer_id: subscription.customer_id || subscription.customer?.id || null,
    customer_email: normalizeEmail(subscription.customer?.email),
    status: subscription.status || null,
    license_status: mapPolarSubscriptionStatus(subscription, event.type),
    plan: polarPlan(subscription, env),
    price_ids: subscription.product_id ? [subscription.product_id] : [],
...
// Polar keeps status "active" while a cancellation waits for the period end
// and only flips it when access really stops, so the status field is enough.
function mapPolarSubscriptionStatus(subscription, eventType) {
  if (eventType === "subscription.revoked") return "canceled";
...
// Checkout-link metadata (plan=lifetime) rides on every order; product IDs are
// the fallback for orders created without it.
function polarPlan(entity, env) {
  return (
    entity.metadata?.plan ||
    entity.product?.metadata?.plan ||
    inferPolarPlan(entity.product_id || entity.product?.id, env)
  );
}

function inferPolarPlan(productId, env) {
  if (!productId) return "unknown";
  if (productId === env.POLAR_LIFETIME_PRODUCT_ID) return "lifetime";
  if (productId === env.POLAR_MONTHLY_PRODUCT_ID) return "monthly";
  if (productId === env.POLAR_YEARLY_PRODUCT_ID) return "yearly";
  return "unknown";
}
...
async function ensureLicenseForSubscription(subscriptionSummary, subscription, env) {
  ...
  if (!license) {
    license = createLicenseRecord({
      source: subscriptionSummary.source || "paddle",
      status: nextStatus,
...
async function verifyPolarSignature({
  ...
  const nowSeconds = Math.floor(Date.now() / 1000);
  const eventSeconds = Number(webhookTimestamp);
  if (!Number.isFinite(eventSeconds)) {
    return { ok: false, error: "invalid_signature_timestamp" };
  }
  ...
  for (const keyBytes of polarSigningKeys(secret)) {
    const expectedSignature = await hmacSha256Base64(keyBytes, signedContent);
...
// Standard Webhooks key first (decoded secret), legacy Polar key second (the
// secret string's own bytes).
function polarSigningKeys(secret) {
  const trimmed = String(secret).trim();
  const encoded = trimmed.startsWith("whsec_") ? trimmed.slice("whsec_".length) : trimmed;
  const keys = [];

  try {
    keys.push(bytesFromBase64(encoded));
  } catch {
    // Not base64: only the raw-string key can apply.
  }

  keys.push(new TextEncoder().encode(trimmed));
  return keys;
}
