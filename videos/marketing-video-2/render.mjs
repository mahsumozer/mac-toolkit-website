...
const OUT_DIR = join(ROOT, "marketing-video-2", "out");
const VIDEO_OUT = join(OUT_DIR, "mac-kit-launch-promo-2.mp4");
const POSTER_OUT = join(OUT_DIR, "mac-kit-launch-poster-2.png");
...
// ---- Camera: a short push into the preview corner once the shot lands ----
const ZOOM_MAX = 1.38;
const ZOOM_IN = [6.95, 7.6];
const ZOOM_OUT = [11.45, 12.05];
function camera(outT) {
  const p = Math.min(
  ...
  const hx = WIDTH / 2 / z;
  const hy = HEIGHT / 2 / z;
  const fx = clamp(lerp(WIDTH / 2, CARD_RIGHT_X + CARD_W / 2, p), hx, WIDTH - hx);
  const fy = clamp(lerp(HEIGHT / 2, CARD_BOTTOM_Y + CARD_H / 2, p), hy, HEIGHT - hy);
  return [
...
function frameSvg(index) {
  ...
  const [camOpen, camClose] = camera(outT);
