...
const OUT_DIR = join(ROOT, "marketing-video-window-full", "out");
const VIDEO_OUT = join(OUT_DIR, "mac-kit-window-full-promo.mp4");
const POSTER_OUT = join(OUT_DIR, "mac-kit-window-full-poster.png");
...
const NAV = [
  ...
  [14.6 + INTRO, "color"],
  [16.2 + INTRO, "pomodoro"],
  [17.8 + INTRO, "screendraw"],
];
// Index of each page inside the sidebar nav list (for the active pill position).
const PAGE_IDX = { home: 0, screenshot: 1, clipboard: 2, monitor: 3, keepawake: 4, newfile: 5, convert: 6, color: 7, pomodoro: 8, screendraw: 9 };
...
function pagePomodoro(t) {
  ...
  const remain = Math.max(0, 1050 - Math.max(0, Math.floor(t - (16.2 + INTRO))));
...
// ---- Extra icons for the additional tool pages ----
function iconLink(x, y, size = 18, color = W_ACCENT) {
  return icon(x, y, size, color, `<path d="M8 12 L12 8"/><path d="M7 9 L5 11 A2.8 2.8 0 0 0 9 15 L11 13"/><path d="M13 11 L15 9 A2.8 2.8 0 0 0 11 5 L9 7"/>`);
}
function iconMail(x, y, size = 18, color = W_ACCENT) {
  return icon(x, y, size, color, `<rect x="3" y="5" width="14" height="10" rx="2"/><path d="M3.5 6 L10 11 L16.5 6"/>`);
}
function iconText(x, y, size = 18, color = W_ACCENT) {
  return icon(x, y, size, color, `<line x1="5" y1="6" x2="15" y2="6"/><line x1="5" y1="10" x2="15" y2="10"/><line x1="5" y1="14" x2="11" y2="14"/>`);
}
function iconFolder(x, y, size = 18, color = W_ACCENT) {
  return icon(x, y, size, color, `<path d="M3 6 A1.5 1.5 0 0 1 4.5 4.5 H8 L10 6.5 H15.5 A1.5 1.5 0 0 1 17 8 V14 A1.5 1.5 0 0 1 15.5 15.5 H4.5 A1.5 1.5 0 0 1 3 14 Z"/>`);
}
function iconActivity(x, y, size = 18, color = W_ACCENT) {
  return icon(x, y, size, color, `<path d="M2 10 H6 L8 5 L12 15 L14 10 H18"/>`);
}
function iconArrow(x, y, size = 18, color = W_T2) {
  return icon(x, y, size, color, `<line x1="4" y1="10" x2="16" y2="10"/><path d="M11 5 L16 10 L11 15"/>`);
}
function iconBox(x, y, size = 18, color = W_T2) {
  return icon(x, y, size, color, `<rect x="4" y="5" width="12" height="10" rx="2"/>`);
}
...
function pageClipboard(t) {
  let s = pageTitle("Clipboard History", "Everything you copied, one search away.");
  s += `<rect x="224" y="122" width="612" height="42" rx="11" fill="rgba(0,0,0,0.2)" stroke="rgba(255,255,255,0.09)"/>`;
  s += `<circle cx="248" cy="141" r="7" fill="none" stroke="${W_T3}" stroke-width="2"/>`;
  s += `<line x1="253" y1="146" x2="258" y2="151" stroke="${W_T3}" stroke-width="2" stroke-linecap="round"/>`;
  ...
  entries.forEach(([type, val, ts], i) => {
    const ry = 182 + i * 64;
    s += `<rect x="224" y="${ry}" width="612" height="54" rx="12" fill="${W_CARD}" stroke="${W_CARDB}"/>`;
    s += `<rect x="240" y="${ry + 11}" width="32" height="32" rx="9" fill="${W_ACCENT_SUB}"/>`;
    if (type === "color") {
      s += `<rect x="246" y="${ry + 17}" width="20" height="20" rx="6" fill="${val}" stroke="rgba(255,255,255,0.2)"/>`;
    } else {
      const glyph = type === "link" ? iconLink : type === "mail" ? iconMail : type === "file" ? iconFilePlus : iconText;
      s += glyph(247, ry + 18, 18, W_ACCENT);
    }
    const isText = type === "text";
    s += text(val, 290, ry + 32, 14, { weight: 600, fill: isText ? W_T1 : "#c8c3cd", family: isText ? sans : mono });
    s += text(ts, 770, ry + 32, 11.5, { weight: 600, fill: W_T3, anchor: "end" });
    s += iconCopy(804, ry + 20, 15, "#8f8b96");
  });
  return s;
}
function pageSystemMonitor(t) {
  let s = pageTitle("System Monitor", "CPU, memory and network at a glance.");
  const stats = [
    ["CPU", "38%", 0.38, W_ACCENT, "4 cores · 2.1 GHz"],
    ["Memory", "9.8 GB", 0.61, W_GREEN, "of 16 GB used"],
    ["Disk", "212 GB", 0.41, "#5AA9FF", "free of 512 GB"],
  ];
  stats.forEach(([label, val, pct, col, sub], i) => {
    const x = 224 + i * 205;
    s += winCard(x, 122, 192, 150);
    s += text(label.toUpperCase(), x + 18, 150, 11, { weight: 800, fill: W_T3, spacing: 1.3 });
    s += text(val, x + 18, 192, 30, { weight: 900, fill: W_T1 });
    s += `<rect x="${x + 18}" y="208" width="156" height="7" rx="3.5" fill="rgba(255,255,255,0.1)"/>`;
    s += `<rect x="${x + 18}" y="208" width="${(156 * pct).toFixed(0)}" height="7" rx="3.5" fill="${col}"/>`;
    s += text(sub, x + 18, 246, 11, { weight: 500, fill: W_T3 });
  });
  s += winCard(224, 288, 612, 120);
  s += cardHeader(224, 288, iconActivity, "Network Activity");
  const spark = "M244 374 L280 356 L316 368 L352 338 L388 360 L424 330 L460 352 L496 336 L532 362 L568 342 L604 372";
  s += `<path d="${spark}" fill="none" stroke="${W_ACCENT}" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>`;
  s += text("DOWNLOAD", 636, 342, 10.5, { weight: 800, fill: W_T3, spacing: 1 });
  s += text("4.2 MB/s", 636, 372, 19, { weight: 900, fill: W_GREEN });
  s += text("UPLOAD", 748, 342, 10.5, { weight: 800, fill: W_T3, spacing: 1 });
  s += text("1.1 MB/s", 748, 372, 19, { weight: 900, fill: "#5AA9FF" });
  s += winCard(224, 424, 612, 132);
  s += text("TOP PROCESSES", 244, 452, 11, { weight: 800, fill: W_T3, spacing: 1.3 });
  const procs = [["Final Cut Pro", "24.3%", "3.1 GB"], ["Google Chrome", "12.8%", "1.8 GB"], ["Xcode", "8.1%", "2.4 GB"]];
  procs.forEach(([n, c, m], i) => {
    const ry = 480 + i * 26;
    s += text(n, 244, ry, 13, { weight: 600, fill: W_T1 });
    s += text(c, 660, ry, 12.5, { weight: 700, fill: W_ACCENT, anchor: "end", family: mono });
    s += text(m, 816, ry, 12.5, { weight: 600, fill: W_T2, anchor: "end", family: mono });
  });
  return s;
}
function pageKeepAwake(t) {
  let s = pageTitle("Keep Awake", "Stop your Mac from sleeping.");
  s += winCard(224, 122, 612, 176);
  s += `<circle cx="304" cy="210" r="44" fill="${W_ACCENT_SUB}"/>`;
  s += iconCoffee(304 - 22, 210 - 22, 44, W_ACCENT);
  s += text("Awake", 376, 196, 24, { weight: 900, fill: W_T1 });
  s += text("Your Mac will stay awake indefinitely.", 376, 226, 13.5, { weight: 500, fill: W_T2 });
  s += `<circle cx="382" cy="254" r="4" fill="${W_GREEN}"/>`;
  s += text("Active since 10:20", 396, 258, 12, { weight: 600, fill: W_T3 });
  s += `<rect x="700" y="184" width="108" height="52" rx="26" fill="${W_ACCENT}"/>`;
  s += `<circle cx="782" cy="210" r="20" fill="#ffffff"/>`;
  s += text("ON", 728, 215, 14, { weight: 900, fill: "#16131a" });
  s += text("DURATION", 224, 336, 11, { weight: 800, fill: W_T3, spacing: 1.3 });
  const durs = [["Indefinitely", true], ["1 hour", false], ["2 hours", false], ["Until I quit", false]];
  let dx = 224;
  durs.forEach(([lab, active]) => {
    const w = 26 + lab.length * 8.4;
    s += `<rect x="${dx}" y="350" width="${w.toFixed(0)}" height="40" rx="11" fill="${active ? W_ACCENT_SUB : "rgba(255,255,255,0.04)"}" stroke="${active ? "rgba(255,159,28,0.4)" : "rgba(255,255,255,0.08)"}"/>`;
    s += text(lab, dx + w / 2, 375, 13, { anchor: "middle", weight: active ? 700 : 600, fill: active ? W_ACCENT : W_T2 });
    dx += w + 12;
  });
  const opts2 = [["Prevent display sleep", true], ["Keep network alive", true], ["Pause when on battery", false]];
  opts2.forEach(([lab, on], i) => {
    const ry = 416 + i * 44;
    s += `<rect x="224" y="${ry}" width="612" height="36" rx="10" fill="${W_CARD}" stroke="${W_CARDB}"/>`;
    s += text(lab, 244, ry + 23, 13, { weight: 600, fill: W_T1 });
    s += `<rect x="770" y="${ry + 8}" width="46" height="22" rx="11" fill="${on ? W_ACCENT : "rgba(255,255,255,0.12)"}"/>`;
    s += `<circle cx="${on ? 805 : 781}" cy="${ry + 19}" r="8" fill="#ffffff"/>`;
  });
  return s;
}
function pageNewFile(t) {
  let s = pageTitle("New File", "Create files anywhere, instantly.");
  const tpl = [
    ["Markdown", ".md", "#5AA9FF"], ["Plain Text", ".txt", W_T2], ["JavaScript", ".js", "#f7df1e"],
    ["Python", ".py", "#22c55e"], ["JSON", ".json", W_ACCENT], ["Shell", ".sh", "#a855f7"],
  ];
  tpl.forEach(([name, ext, col], i) => {
    const ci = i % 3, row = Math.floor(i / 3);
    const x = 224 + ci * 205;
    const y = 122 + row * 104;
    const active = i === 0;
    s += `<rect x="${x}" y="${y}" width="192" height="92" rx="12" fill="${active ? W_ACCENT_SUB : W_CARD}" stroke="${active ? "rgba(255,159,28,0.4)" : W_CARDB}"/>`;
    s += iconFilePlus(x + 16, y + 16, 26, col);
    s += text(name, x + 52, y + 36, 14, { weight: 700, fill: W_T1 });
    const bw = 16 + ext.length * 8;
    s += `<rect x="${x + 16}" y="${y + 54}" width="${bw}" height="22" rx="6" fill="rgba(0,0,0,0.24)"/>`;
    s += text(ext, x + 16 + bw / 2, y + 69, 12, { anchor: "middle", weight: 800, fill: col, family: mono });
  });
  s += `<rect x="224" y="350" width="612" height="44" rx="11" fill="rgba(0,0,0,0.2)" stroke="rgba(255,255,255,0.1)"/>`;
  s += text("untitled.md", 244, 378, 15, { weight: 700, fill: W_T1, family: mono });
  s += text("NAME", 816, 378, 11, { weight: 800, fill: W_T3, anchor: "end", spacing: 1 });
  s += `<rect x="224" y="406" width="612" height="44" rx="11" fill="${W_CARD}" stroke="${W_CARDB}"/>`;
  s += iconFolder(240, 419, 18, W_ACCENT);
  s += text("~/Desktop", 272, 434, 14, { weight: 600, fill: W_T1 });
  s += `<rect x="742" y="414" width="78" height="28" rx="8" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.1)"/>`;
  s += text("Browse", 781, 433, 12, { anchor: "middle", weight: 700, fill: W_T2 });
  s += `<rect x="224" y="466" width="612" height="48" rx="12" fill="${W_ACCENT}"/>`;
  s += text("Create File", 530, 496, 16, { anchor: "middle", weight: 800, fill: "#16131a" });
  return s;
}
function pageConvert(t) {
  let s = pageTitle("Convert", "Images and files to any format.");
  s += `<rect x="224" y="122" width="270" height="60" rx="13" fill="${W_CARD}" stroke="${W_CARDB}"/>`;
  s += text("FROM", 244, 146, 10.5, { weight: 800, fill: W_T3, spacing: 1.3 });
  s += text("PNG", 244, 172, 20, { weight: 900, fill: W_T1 });
  s += text("image · 2.4 MB", 320, 172, 12, { weight: 500, fill: W_T3 });
  s += iconConvert(508, 138, 28, W_ACCENT);
  s += `<rect x="566" y="122" width="270" height="60" rx="13" fill="${W_ACCENT_SUB}" stroke="rgba(255,159,28,0.4)"/>`;
  s += text("TO", 586, 146, 10.5, { weight: 800, fill: W_ACCENT, spacing: 1.3 });
  s += text("JPG", 586, 172, 20, { weight: 900, fill: W_ACCENT });
  s += text("~ 640 KB", 662, 172, 12, { weight: 500, fill: W_T2 });
  s += `<rect x="224" y="200" width="612" height="150" rx="14" fill="rgba(255,255,255,0.02)" stroke="rgba(255,255,255,0.16)" stroke-dasharray="8 7"/>`;
  s += iconFilePlus(513, 240, 34, W_T2);
  s += text("Drop files here", 530, 302, 17, { anchor: "middle", weight: 800, fill: W_T1 });
  s += text("or click to browse — PNG, JPG, HEIC, PDF, WEBP", 530, 328, 12.5, { anchor: "middle", weight: 500, fill: W_T3 });
  s += text("QUALITY", 224, 384, 11, { weight: 800, fill: W_T3, spacing: 1.3 });
  s += `<rect x="224" y="396" width="500" height="6" rx="3" fill="rgba(255,255,255,0.1)"/>`;
  s += `<rect x="224" y="396" width="450" height="6" rx="3" fill="${W_ACCENT}"/>`;
  s += `<circle cx="674" cy="399" r="10" fill="#ffffff"/>`;
  s += text("90%", 816, 403, 14, { weight: 800, fill: W_T1, anchor: "end" });
  s += `<rect x="224" y="430" width="612" height="48" rx="12" fill="${W_ACCENT}"/>`;
  s += text("Convert 3 files", 530, 460, 16, { anchor: "middle", weight: 800, fill: "#16131a" });
  s += text("RECENT", 224, 510, 10.5, { weight: 800, fill: W_T3, spacing: 1.3 });
  s += text("shot-01.png → shot-01.jpg   ·   logo.heic → logo.png", 296, 510, 12, { weight: 500, fill: W_T3, family: mono });
  return s;
}
function pageScreenDraw(t) {
  let s = pageTitle("Screen Draw", "Annotate and present right on screen.");
  s += winCard(224, 122, 612, 60);
  const tls = [["Pen", iconPencil], ["Arrow", iconArrow], ["Box", iconBox], ["Mark", iconBrush], ["Text", iconText], ["Erase", iconEraser]];
  let tx = 240;
  tls.forEach(([, ic], i) => {
    const active = i === 1;
    s += `<rect x="${tx}" y="134" width="36" height="36" rx="10" fill="${active ? W_ACCENT_SUB : "rgba(255,255,255,0.05)"}" stroke="${active ? "rgba(255,159,28,0.4)" : "rgba(255,255,255,0.08)"}"/>`;
    s += ic(tx + 9, 143, 18, active ? W_ACCENT : W_T2);
    tx += 44;
  });
  s += `<line x1="${tx + 6}" y1="138" x2="${tx + 6}" y2="166" stroke="rgba(255,255,255,0.12)"/>`;
  const cols = [W_ACCENT, "#ef4444", "#22c55e", "#5AA9FF", "#ffffff"];
  cols.forEach((c, i) => {
    s += `<circle cx="${tx + 28 + i * 34}" cy="152" r="11" fill="${c}" stroke="${i === 0 ? "#ffffff" : "rgba(255,255,255,0.25)"}" stroke-width="${i === 0 ? 2 : 1}"/>`;
  });
  s += `<rect x="224" y="200" width="612" height="330" rx="14" fill="rgba(10,12,20,0.35)" stroke="rgba(255,255,255,0.09)"/>`;
  s += `<rect x="256" y="238" width="300" height="180" rx="10" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.08)"/>`;
  s += `<circle cx="272" cy="254" r="4" fill="#ff5f57"/><circle cx="286" cy="254" r="4" fill="#febc2e"/><circle cx="300" cy="254" r="4" fill="#28c840"/>`;
  s += `<rect x="272" y="278" width="180" height="12" rx="6" fill="rgba(255,255,255,0.14)"/>`;
  s += `<rect x="272" y="302" width="240" height="10" rx="5" fill="rgba(255,255,255,0.09)"/>`;
  s += `<rect x="272" y="322" width="200" height="10" rx="5" fill="rgba(255,255,255,0.09)"/>`;
  s += `<circle cx="470" cy="366" r="46" fill="none" stroke="${W_ACCENT}" stroke-width="4"/>`;
  s += `<path d="M668 300 Q560 322 522 350" fill="none" stroke="#ef4444" stroke-width="4" stroke-linecap="round"/>`;
  s += `<path d="M522 350 L540 344 M522 350 L534 362" fill="none" stroke="#ef4444" stroke-width="4" stroke-linecap="round"/>`;
  s += text("Look here!", 672, 294, 20, { weight: 900, fill: "#ef4444" });
  return s;
}
function renderPage(page, t) {
  if (page === "screenshot") return pageScreenshot(t);
  if (page === "clipboard") return pageClipboard(t);
  if (page === "monitor") return pageSystemMonitor(t);
  if (page === "keepawake") return pageKeepAwake(t);
  if (page === "newfile") return pageNewFile(t);
  if (page === "convert") return pageConvert(t);
  if (page === "color") return pageColor(t);
  if (page === "pomodoro") return pagePomodoro(t);
  if (page === "screendraw") return pageScreenDraw(t);
  return pageHome(t);
}
...
// ---- Cursor ----
// Screen-space Y of sidebar nav row `idx`: 253.8 + idx*48.6 (window at WY=108, scale 1.35).
const CURSOR_KEYS = [
  [0.0, 950, 1000],
  ...
  [5.0 + INTRO, 515, 302], // click Screenshot (row 1)
  [6.45 + INTRO, 515, 302], // rest through dwell
  [6.6 + INTRO, 515, 351], // click Clipboard (row 2)
  [8.05 + INTRO, 515, 351],
  [8.2 + INTRO, 515, 400], // click System Monitor (row 3)
  [9.65 + INTRO, 515, 400],
  [9.8 + INTRO, 515, 448], // click Keep Awake (row 4)
  [11.25 + INTRO, 515, 448],
  [11.4 + INTRO, 515, 497], // click New File (row 5)
  [12.85 + INTRO, 515, 497],
  [13.0 + INTRO, 515, 545], // click Convert (row 6)
  [14.45 + INTRO, 515, 545],
  [14.6 + INTRO, 515, 594], // click Color Picker (row 7)
  [16.05 + INTRO, 515, 594],
  [16.2 + INTRO, 515, 643], // click Pomodoro (row 8)
  [17.65 + INTRO, 515, 643],
  [17.8 + INTRO, 515, 691], // click Screen Draw (row 9)
  [19.25 + INTRO, 515, 691],
  [19.6 + INTRO, 830, 470], // drift into the content
];
const CLICK_TIMES = [
  POP_OPEN, POP_CLOSE, 1.4 + INTRO, 3.3 + INTRO,
  5.0 + INTRO, 6.6 + INTRO, 8.2 + INTRO, 9.8 + INTRO, 11.4 + INTRO,
  13.0 + INTRO, 14.6 + INTRO, 16.2 + INTRO, 17.8 + INTRO,
];
...
function cursorLayer(t) {
  const op = fade(t, 0.0, 0.5, 19.1 + INTRO, 19.7 + INTRO);
  if (op <= 0) return "";
...
function captions(t) {
  const list = [
    ...
    [3.6 + INTRO, 4.8 + INTRO, "Open the whole toolkit in a window."],
    [4.9 + INTRO, 6.4 + INTRO, "Capture or record your screen."],
    [6.5 + INTRO, 8.0 + INTRO, "Clipboard history, always a click away."],
    [8.1 + INTRO, 9.6 + INTRO, "Watch CPU, memory and network live."],
    [9.7 + INTRO, 11.2 + INTRO, "Keep your Mac awake on demand."],
    [11.3 + INTRO, 12.8 + INTRO, "Create files anywhere in a flash."],
    [12.9 + INTRO, 14.4 + INTRO, "Convert images to any format."],
    [14.5 + INTRO, 16.0 + INTRO, "Pick colors from anywhere."],
    [16.1 + INTRO, 17.6 + INTRO, "Focus with a built-in timer."],
    [17.7 + INTRO, 19.2 + INTRO, "Draw on screen to present."],
  ];
...
function cta(t) {
  ...
  const bgOp = clamp((t - (19.4 + INTRO)) / 0.25, 0, 1);
  if (bgOp <= 0) return "";
  const accent = "#f5941d";
  const pillOpts = { fill: "#ffffff", color: "#6d6b66", border: "rgba(5,5,5,0.12)" };
  const pulse = 0.5 + 0.5 * Math.sin((t - (19.7 + INTRO)) * Math.PI * 2 / 1.2);
  ...
  const rev = (delay) => {
    const p = easeOutCubic(clamp((t - (19.7 + INTRO + delay)) / 0.5, 0, 1));
